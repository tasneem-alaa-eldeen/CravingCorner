<x-app-layout>
    <x-slot name="header">
        <h2 class="font-display" style="font-size:1.4rem; font-weight:700; color: var(--cc-ink);">{{ $beverage->name }}</h2>
    </x-slot>

    <div class="py-8 max-w-xl mx-auto sm:px-6 lg:px-8">
        <div class="card-ticket" style="padding:1.5rem;">
            @if ($beverage->image)
                <img src="{{ asset('storage/' . $beverage->image) }}" style="width:100%; height:220px; object-fit:contain; border-radius:8px; margin-bottom:1rem; background: var(--cc-paper); border:1px solid var(--cc-line); padding:0.6rem;" alt="{{ $beverage->name }}">
            @else
                <div style="width:100%; height:220px; border-radius:8px; margin-bottom:1rem; background: var(--cc-paper); border:1px dashed var(--cc-line); display:flex; align-items:center; justify-content:center; font-size:3rem;">🥤</div>
            @endif
            <p style="font-size:0.8rem; color: var(--cc-text-muted);">{{ $beverage->category->name ?? '' }}</p>
            @if ($avgRating)
                <p style="font-size:0.875rem; color: var(--cc-mustard-dark); margin-top:0.25rem;">
                    ...
                    <span style="color: var(--cc-text-muted);">({{ number_format($avgRating, 1) }} / 5, {{ $beverage->reviews->count() }} reviews)</span>
                </p>
            @else
                <p style="font-size:0.875rem; color: var(--cc-text-muted); margin-top:0.25rem;">No reviews yet.</p>
            @endif

            <p style="margin-top:0.75rem; color: var(--cc-ink);">{{ $beverage->description }}</p>

            <dl style="display:grid; grid-template-columns:1fr 1fr; gap:0.75rem; font-size:0.875rem; margin-top:1rem;">
                <div><dt style="color: var(--cc-text-muted);">Price</dt><dd class="font-mono">{{ number_format($beverage->price, 2) }} EGP</dd></div>
                <div><dt style="color: var(--cc-text-muted);">Calories</dt><dd>{{ $beverage->calories ?? '—' }}</dd></div>
                <div><dt style="color: var(--cc-text-muted);">Temperature</dt><dd>{{ ucfirst($beverage->temperature) }}</dd></div>
                <div><dt style="color: var(--cc-text-muted);">Size</dt><dd>{{ $beverage->size ?? '—' }}</dd></div>
            </dl>

            @if ($beverage->ingredients)
                <p style="margin-top:0.75rem; font-size:0.875rem;"><span style="color: var(--cc-text-muted);">Ingredients:</span> {{ $beverage->ingredients }}</p>
            @endif
