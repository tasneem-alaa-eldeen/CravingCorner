            ...
            @forelse ($foodItems as $item)
                <div class="bg-white shadow-sm rounded-lg p-4">
                    ...
                    <form action="{{ route('cart.add') }}" method="POST" class="mt-3 flex items-center gap-2">
                        ...
                        <input type="number" name="quantity" value="1" min="1" class="w-16 border-gray-300 rounded-md text-sm">
                        <button class="px-3 py-1.5 bg-indigo-600 text-white rounded-md text-xs">Add to cart</button>
                    </form>
                </div>
            @empty
