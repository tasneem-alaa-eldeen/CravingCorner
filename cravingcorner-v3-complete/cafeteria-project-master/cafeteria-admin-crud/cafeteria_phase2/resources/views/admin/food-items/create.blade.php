<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">New Food Item</h2>
    </x-slot>

    <div class="py-8 max-w-2xl mx-auto sm:px-6 lg:px-8">
        <form action="{{ route('admin.food-items.store') }}" method="POST" class="bg-white shadow-sm sm:rounded-lg p-6 space-y-4">
            @csrf
            <div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <x-text-input id="calories" name="calories" type="number" class="mt-1 block w-full" value="{{ old('calories') }}" />
                </div>
                <div>
                    <x-input-label for="spicy_level" value="Spicy level (0-5)" />
                    <x-text-input id="spicy_level" name="spicy_level" type="number" min="0" max="5" class="mt-1 block w-full" value="{{ old('spicy_level', 0) }}" required />
                </div>
                <div>
                    <x-input-label for="preparation_time" value="Prep time (min)" />
                    <x-text-input id="preparation_time" name="preparation_time" type="number" class="mt-1 block w-full" value="{{ old('preparation_time') }}" />
                </div>
                <div>
                    <x-input-label for="available_quantity" value="Available quantity" />
                    <x-text-input id="available_quantity" name="available_quantity" type="number" class="mt-1 block w-full" value="{{ old('available_quantity', 0) }}" required />
                </div>
                <div>
            ...
            <div>
                ...
                <textarea id="ingredients" name="ingredients" rows="2" class="mt-1 block w-full border-gray-300 rounded-md">{{ old('ingredients') }}</textarea>
            </div>
