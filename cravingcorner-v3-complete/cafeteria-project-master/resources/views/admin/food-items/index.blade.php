<x-app-layout>
    <x-slot name="header">
        <h2 class="font-display" style="font-size:1.4rem; font-weight:700; color: var(--cc-ink);">Food Items</h2>
    </x-slot>
    ...
    <div class="py-8 max-w-5xl mx-auto sm:px-6 lg:px-8">
        ...
        <div style="display:flex; justify-content:flex-end; margin-bottom:1rem;">
            <a href="{{ route('admin.food-items.create') }}" class="btn-mustard">+ New Food Item</a>
        </div>
                ...
                <tbody>
                    @forelse ($foodItems as $item)
                        <tr style="border-bottom:1px solid var(--cc-line);">
                            ...
                            <td style="padding:0.75rem 1.1rem;">
                                <span class="badge {{ $item->status === 'active' ? 'badge-sage' : 'badge-muted' }}">{{ $item->status }}</span>
                            </td>
                            <td style="padding:0.75rem 1.1rem; text-align:right; white-space:nowrap;">
                                <a href="{{ route('admin.food-items.edit', $item) }}" style="color: var(--cc-mustard-dark); font-weight:600; font-size:0.8125rem; text-decoration:none; margin-right:0.75rem;">Edit</a>
                                <form action="{{ route('admin.food-items.destroy', $item) }}" method="POST" style="display:inline;" onsubmit="return confirm('Delete this item?')">
                                    @csrf @method('DELETE')
                    ...
                    @empty
                        <tr><td colspan="7" style="padding:2rem; text-align:center; color: var(--cc-text-muted);">No food items yet.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
